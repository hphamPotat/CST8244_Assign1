...
//		rcvid = MsgReceive(controllerChid, &person, sizeof(Person), NULL);
		rcvid = MsgReceive(controllerAttach->chid, &person, sizeof(Person),
				NULL);
...
void *doorScanState() {
	...
	if ((strcmp(person.event, inMessage[RS]) == 0
			|| strcmp(person.event, inMessage[LS]) == 0)
			&& display.person.direction == DEFAULT) {
		display.person = person;
		display.indexOutMessage = OUT_LS_RS;

		if (MsgSend(controllerCoid, &display, sizeof(Display), &display,
				sizeof(Display)) == -1L) {
...
void *guardFirstDoorUnlockState() {
	...
	if (strcmp(person.event, inMessage[GLU]) == 0
			&& display.person.direction == LEFT) {
		strcpy(display.person.event, person.event);
//		display.person.event = person.event;
		display.indexOutMessage = OUT_GLU;

	} else if (strcmp(person.event, inMessage[GRU]) == 0
			&& display.person.direction == RIGHT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
	...
	if (MsgSend(controllerCoid, &display, sizeof(Display), &display,
			sizeof(Display)) == -1L) {
		perror(
				"Controller failed to send message (guardFirstDoorUnlockState)\n");
		exit(EXIT_FAILURE);
	}
...
void *firstDoorOpenState() {
	...
	if (strcmp(person.event, inMessage[LO]) == 0
			&& display.person.direction == LEFT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_LO;

	} else if (strcmp(person.event, inMessage[RO]) == 0
			&& display.person.direction == RIGHT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
...
void *firstDoorCloseState() {
	...
	if (strcmp(person.event, inMessage[LC]) == 0
			&& display.person.direction == LEFT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_LC;

	} else if (strcmp(person.event, inMessage[RC]) == 0
			&& display.person.direction == RIGHT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
...
void *guardFirstDoorLockState() {
	...
	if (strcmp(person.event, inMessage[GLL]) == 0
			&& display.person.direction == LEFT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_GLL;

	} else if (strcmp(person.event, inMessage[GRL]) == 0
			&& display.person.direction == RIGHT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
...
void *guardSecondDoorUnlockState() {
	...
	if (strcmp(person.event, inMessage[GRU]) == 0
			&& display.person.direction == LEFT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_GRU;

	} else if (strcmp(person.event, inMessage[GLU]) == 0
			&& display.person.direction == RIGHT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_GLU;
	...
	if (MsgSend(controllerCoid, &display, sizeof(Display), &display,
			sizeof(Display)) == -1L) {
		perror(
				"Controller failed to send message (guardSecondDoorUnlockState)\n");
		exit(EXIT_FAILURE);
	}
...
void *secondDoorOpenState() {
	...
	if (strcmp(person.event, inMessage[RO]) == 0
			&& display.person.direction == LEFT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_RO;

	} else if (strcmp(person.event, inMessage[LO]) == 0
			&& display.person.direction == RIGHT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_LO;
...
void *secondDoorCloseState() {
	...
	if (strcmp(person.event, inMessage[RC]) == 0
			&& display.person.direction == LEFT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_RC;

	} else if (strcmp(person.event, inMessage[LC]) == 0
			&& display.person.direction == RIGHT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_LC;
...
void *guardSecondDoorLockState() {
	...
	if (strcmp(person.event, inMessage[GRL]) == 0
			&& display.person.direction == LEFT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_GRL;

	} else if (strcmp(person.event, inMessage[GLL]) == 0
			&& display.person.direction == RIGHT) {
		strcpy(display.person.event, person.event);
		//		display.person.event = person.event;
		display.indexOutMessage = OUT_GLL;
	...
	if (MsgSend(controllerCoid, &display, sizeof(Display), &display,
			sizeof(Display)) == -1L) {
		perror(
				"Controller failed to send message (guardSecondDoorLockState)\n");
		exit(EXIT_FAILURE);
	}
